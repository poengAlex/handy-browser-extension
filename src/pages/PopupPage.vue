    ...
    <div class="text-center full-width q-mt-sm q-mb-sm">
      ...
      <template v-else>
        <template v-if="connected">
          ...
          <q-btn @click="handy.disconnect()">Disconnect</q-btn>
        </template>
        <template v-else>
          <q-input v-model="connectionKey" @keydown.enter="connect"></q-input>
    ...
    <q-list padding>
      ...
      <q-item v-if="state.scriptTokenUrl !== ''">
        ...
        <q-item-section side top v-if="state.scriptSet">
          ...
          <q-tooltip>
            Set the script once more on your Handy. Click here if the video is not playing the script correctly.
          </q-tooltip>
        </q-item-section>
      </q-item>
      <q-item v-else>
...
function refreshScript() {
  $q.bex.send('log', {
    message: '[popup]: Refresh script on Handy'
  });
}
